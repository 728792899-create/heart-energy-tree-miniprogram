...
| 线上云函数 | 向后兼容的 `3.1.0` 候选 `energyTree` 已于 2026-07-23 部署；模拟器真实 `queryDashboard` 已通过 buildTag + releaseTag 校验 |
| 线上微信版本 | `3.0.0` 已于 2026-07-23 13:09:40 正式发布（依据管理员提供的平台截图） |
| 下一候选 | `3.1.0` 增加双方确认、两次警告的关系解除入口；云函数已部署、开发版本已上传，尚未提审或发布 |

`3.1.0` 仍是候选版本，不是当前线上版本；完成隔离云环境双账号验收前不得提审或发布。
...
- 本次部署前已在仓库外完成生产数据库备份：22 个集合均进入脱敏清单，21 个非空集合生成 JSON 与 SHA-256，空集合单独记录，并保存云存储根目录清单；备份文件不会提交到 Git。
