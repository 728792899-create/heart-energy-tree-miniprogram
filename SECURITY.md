...
安全修复只针对 `main` 上的私人版 V3 / 小程序 `3.1.0` 候选代码和当前线上 `3.0.0`。该仓库不提供公共多租户服务，也不支持真实支付。云端保留兼容 buildTag 以避免已上线客户端中断；`3.1.0` 还要求匹配独立 releaseTag 才会启用双方解除流程。
...
- 关系解除必须由不同的两位可信成员分别完成两次警告确认；完成前暂停业务写入，完成后撤销实时投影访问并冻结旧关系。不得提供单方解绑、旧关系复用或客户端直接清库入口。
