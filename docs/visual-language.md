...
当前小程序绑定页使用原生 WXSS 为双人插画添加轻量呼吸、光晕和叶片漂移动效，不依赖视频即可上线。若后续需要用 Gemini / Veo 生成独立 MP4，请继续保持固定两人私人版和无隐私信息边界：

```text
Create a seamless 4-second looping animation for a private couple wellness mini program onboarding hero.
Style: soft morning mist botanical journal, ivory paper texture, sage green leaves, warm terracotta accents, gentle cinematic light, high-end cozy mobile app illustration.
Scene: two abstract non-real human silhouettes or faceless clay-like figures stand beside each other in a protected garden, planting two glowing twin seeds that slowly sprout and intertwine into a tiny heart-shaped sapling.
Motion: very subtle breathing light, drifting leaves, soft paper-card shadow, tiny warm sparkles around the sprout, no fast camera movement, no hard cuts.
Composition: 3:2 horizontal card, safe center subject, plenty of clean margin for a mini program hero card, works as both MP4 and static poster.
Do not include any text, logo, QR code, payment symbol, watermark, real face, real person, private photo, phone UI, money, financial product, leaderboard, or public social network elements.
Output: 1080x720 MP4, seamless loop, plus one clean poster frame.
```

中文可直接补充给模型：画面表达“两个人重新种下一棵只属于彼此的心动能量树”，不要出现真实人物、二维码、微信支付、收益提现、排行榜或任何私人信息；动作要慢、轻、可循环，失败时单帧也必须好看。
